...
for i in range(1, message_count + 1):
    ...
    if message_size > largest_message_size:
        largest_message_size = message_size
        largest_message_index = i

print(f"Największa wiadomość: {largest_message_size} bajtów")

response = pop3_connection.retr(largest_message_index)
largest_message_lines = response[1]
largest_message_content = b'\n'.join(largest_message_lines).decode('utf-8')
print(f"Tresc najwiekszej wiadomosci:\n{largest_message_content}")
